...
@ApplicationScoped
@Path("/generator/thorntail")
public class ProjectThorntailGeneratorResource {
    private static final String THORNTAIL_VERSION = "2.2.0.Final";

    TemplateEngine engine;

    public ProjectThorntailGeneratorResource() {
    ...
    public Response generate(@QueryParam("sv") @DefaultValue(THORNTAIL_VERSION) String thorntailVersion,
        ...
        try (ZipOutputStream zos = new ZipOutputStream(baos)) {
            ...
            zos.write(engine.process("templates/thorntail/pom.tl.xml", context).getBytes());
            zos.closeEntry();
            ...
            if (enableJAXRS(dependencies)) {
                ...
                zos.write(engine.process("templates/thorntail/HelloWorldEndpoint.tl.java", context).getBytes());
                zos.putNextEntry(new ZipEntry(artifactId + fpg.getApplicationPath()));
                zos.write(engine.process("templates/thorntail/RestApplication.tl.java", context).getBytes());
                zos.closeEntry();
            }
        }
